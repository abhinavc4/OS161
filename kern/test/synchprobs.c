...
check_intersection() {
	...
	for (int i = 0; i < NUM_QUADRANTS; i++) {
		failif((quadrant_array[i] > 1), "failed here: collision");
		n += quadrant_array[i];
	}
...
{
	random_yielder(4);
	lock_acquire(testlock); //acquire a teslock
...
int stoplight(int nargs, char **args) {
	...
	for (i = 0; i < NUM_QUADRANTS; i++) { //4 Quadrants initialise
		quadrant_array[i] = 0;
	}

	for (i = 0; i < NCARS; i++) { //64 Cars
		car_locations[i] = UNKNOWN_CAR; //no car s at the locations
		car_threads[i] = NULL; //no threads in the car threads array
		car_directions[i] = -1; //direction of the car is initialised to -1
	}

	startcount = NCARS;  // initialse start count to number of cars
	testlock = lock_create("testlock"); //create a test lock
	if (testlock == NULL) {
		panic("sp2: lock_create failed\n");
	}
	startcv = cv_create("startcv"); //create a condition variable
	if (startcv == NULL) {
		panic("sp2: cv_create failed\n");
	}
	endsem = sem_create("endsem", 0); //an end semaphore
	if (endsem == NULL) {
	...
	stoplight_init(); // initialise the stoplight

	for (i = 0; i < NCARS; i++) {
		kprintf_t(".");

		direction = random() % 4; //four directions / quadrants
		turn = random() % 3;	  //three turns //straight , left , right

		snprintf(name, sizeof(name), "Car Thread %d", i); // create a thread with a name

		switch (turn) {
			case GO_STRAIGHT:
			err = thread_fork(name, NULL, gostraight_wrapper, (void *)i, direction); // create a new thread , call
			required_quadrant += 2;	// it with direction , add value to quadrant
			break;
			case TURN_LEFT:
